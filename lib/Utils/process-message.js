...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        if (reactMsg) {
            try {
                const meLidNormalised = WABinary_1.jidNormalizedUser(meLid)
                const reactionCreatorJid = creationMsgKey.participant ? creationMsgKey.participant : meLidNormalised
                const responderJid = message.key?.participant ? message.key.participant : meLidNormalised
